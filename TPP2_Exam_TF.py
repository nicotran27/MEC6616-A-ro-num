...
class ConvectionDiffusionSolver:
    def __init__(self, x_min, x_max, y_min, y_max, Nx=20, Ny=20):  # Changed lc to Nx, Ny
    ...
    # Maillage
    def generate_mesh(self, mesh_type='QUAD', Nx=None, Ny=None):
        if Nx is not None:
            self.Nx = Nx
        if Ny is not None:
            self.Ny = Ny
        ...
        self.compute_mesh_properties()  # Add this line
        return self.mesh
    ...
    def grad_least_square(self, maillage, metriques, bcdata, phi):
        ...
        # Tableau qui contient les matrices (2,2)
        ATA = np.zeros((Ntri, 2, 2))
        # Tableau qui contient les vecteurs (2,1)
        B = np.zeros((Ntri, 2))
        
        # variation de phi à travers les aretes
        dphi = np.zeros(Nare)
        ...
        # Initialisation des matrices de remplissage
        Ap = np.zeros((2, 2))
        Bp = np.zeros(2)
        ...
        for iare in range(Naref, Nare):
            ...
            Ap[1, 0] = metriques["dxTri"][iare] * metriques["dyTri"][iare]
            Ap[0, 1] = Ap[1, 0]
        ...
        for iaref in range(0, Naref):
            ...
            if bcdata is not None and face_tag < len(bcdata):
                ...
                if bc_type == 'DIRICHLET':
                    # Pour les conditions de Dirichlet
                    dphi[iaref] = bc_value - phi[left_cell]
                    
                    Ap[0, 0] = metriques["dxTri"][iaref] * metriques["dxTri"][iaref]
                    Ap[1, 0] = metriques["dxTri"][iaref] * metriques["dyTri"][iaref]
                    Ap[0, 1] = Ap[1, 0]
                    ...
                    # Pour les conditions de Neumann
                    Ap[0, 0] = metriques["dxTri"][iaref] * metriques["dxTri"][iaref]
                    Ap[1, 0] = metriques["dxTri"][iaref] * metriques["dyTri"][iaref]
                    Ap[0, 1] = Ap[1, 0]
                    Ap[1, 1] = metriques["dyTri"][iaref] * metriques["dyTri"][iaref]
                    ...
                    Bp[0] = metriques["dA"][iaref] * bc_value * normal[0]
                    Bp[1] = metriques["dA"][iaref] * bc_value * normal[1]
                    
                    ATA[left_cell] += Ap
                    B[left_cell] += Bp
        
        # Résolution des systèmes matriciels
        gradPhi = np.zeros((Ntri, 2))
        for itri in range(Ntri):
            try:
                gradPhi[itri] = np.linalg.solve(ATA[itri], B[itri])
            except np.linalg.LinAlgError:
                print(f"Warning: Singular matrix encountered for triangle {itri}")
        ...
        for iaref in range(Naref):
            ...
            if bcdata is not None and face_tag < len(bcdata):
                ...
                if bc_type == 'DIRICHLET':
                    ...
                    x_face, y_face = np.mean([maillage.get_node_to_xycoord(node) for node in face_nodes], axis=0)
                    
                    if callable(bc_value):
                        bcvalue = bc_value(x_face, y_face)
                        # Calculate dbcvalue if needed for variable boundary conditions
                        x1, y1 = maillage.get_node_to_xycoord(face_nodes[0])
                        x2, y2 = maillage.get_node_to_xycoord(face_nodes[1])
                        dbcvalue = bc_value(x2, y2) - bc_value(x1, y1)
                    else:
                        bcvalue = bc_value
                        dbcvalue = 0
                    ...
                    if callable(bc_value):
                        face_nodes = maillage.get_face_to_nodes(iaref)
                        x_face, y_face = np.mean([maillage.get_node_to_xycoord(node) for node in face_nodes], axis=0)
                        neumann_value = bc_value(x_face, y_face)
                    else:
...
class ConvectionDiffusionAnalyzer:
    def __init__(self, x_min, x_max, y_min, y_max, base_N):  # Changed lc to base_N
        self.x_min, self.x_max = x_min, x_max
        self.y_min, self.y_max = y_min, y_max
        self.solver = ConvectionDiffusionSolver(x_min, x_max, y_min, y_max, base_N, base_N)  # Using base_N for both Nx and Ny
        self.mesh = None
        self.centre_cellule = None
    ...
    def generate_mesh(self, nx, ny):
        ...
        self.solver.compute_mesh_properties()  # Add this line
        return self.mesh
    ...
    def run_analysis(self, k_values, schemes, base_N, mesh_sizes, bcdata):
        # Change from:
        # self.generate_mesh(base_N)
        # To:
        self.generate_mesh(base_N, base_N)  # Pass base_N for both nx and ny
    ...
    def run_convergence_study(self, k_values, schemes, mesh_sizes, bcdata):  # Added bcdata parameter
        ...
        for Nx in mesh_sizes:
            self.solver = ConvectionDiffusionSolver(self.x_min, self.x_max, self.y_min, self.y_max, Nx,Nx)
    ...
    def get_analytical_bcdata(self):
        """
        Calculate the correct boundary conditions from the analytical TMMS solution 
        using constant values instead of functions.
        """
        x, y = sp.symbols('x y')
        
        # TMMS solution from the solver
        T_mms = self.solver.T0 + self.solver.Tx * sp.cos(sp.pi * x) + self.solver.Txy * sp.sin(sp.pi * x * y)
        
        # Calculate derivatives for Neumann conditions
        dT_dx = sp.diff(T_mms, x)
        dT_dy = sp.diff(T_mms, y)
        
        # Convert to Python functions
        T_mms_func = sp.lambdify((x, y), T_mms, 'numpy')
        dT_dx_func = sp.lambdify((x, y), dT_dx, 'numpy')
        dT_dy_func = sp.lambdify((x, y), dT_dy, 'numpy')
    
        # Calculate values at the middle of each boundary
        left_value = T_mms_func(-1, 0)    # Middle of left boundary
        right_value = T_mms_func(1, 0)     # Middle of right boundary
        bottom_value = -dT_dy_func(0, -1)  # Middle of bottom boundary, negative for normal direction
        top_value = dT_dy_func(0, 1)       # Middle of top boundary
    
        bcdata = [
        ('DIRICHLET', left_value),    # Left boundary x = -1
        ('DIRICHLET', right_value),   # Right boundary x = 1
        ('NEUMANN', bottom_value),    # Bottom boundary y = -1
        ('NEUMANN', top_value)        # Top boundary y = 1
        ]
        
        print(f"Boundary conditions:")
        print(f"Left (Dirichlet): {left_value}")
        print(f"Right (Dirichlet): {right_value}")
        print(f"Bottom (Neumann): {bottom_value}")
        print(f"Top (Neumann): {top_value}")
        
        return bcdata
        
      
def main():
    base_N = 20  # Base mesh size (number of cells in each direction)
    analyzer = ConvectionDiffusionAnalyzer(-1, 1, -1, 1, base_N)
    
    k_values = [10, 1, 1/10]  # Different diffusion coefficients
    schemes = ['centré', 'upwind']   # Different numerical schemes
    mesh_sizes = np.array([base_N, base_N*2])  # Different mesh refinement levels
    
    # Get analytical boundary conditions
    bcdata = analyzer.get_analytical_bcdata()
    
    analyzer.run_analysis(k_values, schemes, base_N, mesh_sizes, bcdata)
