...
class ConvectionDiffusionSolver:
    ...
    def generate_mesh(self, mesh_type='QUAD', Nx=None, Ny=None):
        ...
        mesh_parameters = {
            'mesh_type': mesh_type,
            'N1': self.Nx,  # Use Nx for radial direction
            'N2': self.Ny   # Use Ny for angular direction
        }
...
class ConvectionDiffusionAnalyzer:
    def __init__(self, x_min, x_max, y_min, y_max, Nx=20, Ny=20):  # Changed lc to Nx, Ny
        self.x_min, self.x_max = x_min, x_max
        self.y_min, self.y_max = y_min, y_max
        self.solver = ConvectionDiffusionSolver(x_min, x_max, y_min, y_max, Nx, Ny)  # Pass Nx, Ny
        self.Nx = Nx  # Store Nx
        self.Ny = Ny  # Store Ny
        self.mesh = None
        self.centre_cellule = None
    ...
    def generate_mesh(self, Nx=None, Ny=None):  # Updated to use Nx, Ny
        if Nx is not None and Ny is not None:
            self.Nx = Nx
            self.Ny = Ny
            self.solver = ConvectionDiffusionSolver(self.x_min, self.x_max, self.y_min, self.y_max, Nx, Ny)
        self.mesh = self.solver.generate_mesh(mesh_type='QUAD')  # Always use QUAD for structured mesh
    ...
    def run_analysis(self, k_values, schemes, base_N, mesh_sizes, bcdata):
        results = []
        for N in mesh_sizes:
            self.generate_mesh(N, N)  # Use same N for both directions
            self.plot_T_mms()
        ...
        self.plot_convergence(mesh_sizes, errors, self.x_max - self.x_min)
        self.print_max_slopes(max_slopes)
        return results

    def run_convergence_study(self, k_values, schemes, mesh_sizes, bcdata):
        errors = {scheme: {error_type: {k: [] for k in k_values} for error_type in ['L1', 'L2', 'Linf']} for scheme in schemes}
        
        for N in mesh_sizes:
            self.solver = ConvectionDiffusionSolver(self.x_min, self.x_max, self.y_min, self.y_max, N, N)
            self.generate_mesh(N, N)
            
            fluide = {"Gamma": 1.0, "Rho": 1.0, "Cp": 1.0}
...
def main():
    base_N = 20
    mesh_sizes = [base_N, base_N*2]
    k_values = [1000, 1]
    schemes = ['centré', 'upwind']
    
    results = []
    for N in mesh_sizes:
        analyzer = ConvectionDiffusionAnalyzer(0, 2, 0, 2, N, N)
        analyzer.generate_mesh(Nx=N, Ny=N)
        ...
        for k in k_values:
            ...
            for scheme in schemes:
                print(f"\nRunning simulation with N={N}, Γ={k}, scheme={scheme}")
                ...
                phi, gradPhi = analyzer.solver.solve_convection_diffusion(mesh, bcdata, fluide, Fi, source, scheme)
                analyzer.plot_outlet_temperature(phi)
                results.append({'mesh_size': N, 'k': k, 'scheme': scheme, 'phi': phi, 'gradPhi': gradPhi})
        
    analyzer.run_analysis(k_values, schemes, N, mesh_sizes, analyzer.get_analytical_bcdata())
