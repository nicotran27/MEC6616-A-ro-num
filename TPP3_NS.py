...
class CouetteFlow:
    ...
    def generate_mesh(self, mesh_type: str = 'TRI', lc: float = 3) -> None:
        """
...
class VelocityPressurePlotter:
    ...
    def _plot_pressure_comparison(self, ax, p: np.ndarray, subtitle: str):
        ...
        x_range = self.solver.x_max - self.solver.x_min
        x_threshold = x_target - 0.1 * x_range
        ...
        for i_elem in range(self.solver.mesh.get_number_of_elements()):
            x, y = self.solver.cell_centers[i_elem]
            if x >= x_threshold:

                y_band = round(y / y_band_width) * y_band_width
                if y_band not in y_bands:
                    y_bands[y_band] = []
                y_bands[y_band].append((x, p[i_elem]))
        ...
        for y_band, points in y_bands.items():
            if len(points) >= 2: 
                points.sort(key=lambda pt: pt[0])
                
      
                x_coords = np.array([pt[0] for pt in points])
                p_values = np.array([pt[1] for pt in points])
           
                A = np.vstack([x_coords, np.ones(len(x_coords))]).T
                m, b = np.linalg.lstsq(A, p_values, rcond=None)[0]
                
                numerical_dpdx.append(m) 
                numerical_y.append(y_band)
        ...
        ax.plot(dpdx_analytical, y_analytical, 'r-', label='Analytique')
        
        if numerical_dpdx:
            ax.scatter(numerical_dpdx, numerical_y, c='b', marker='o', s=100, label='Numérique')
        ...
        textstr = f'Gradient théorique dp/dx = {dpdx_theory:.2f}'
        ...
        if numerical_dpdx:
            data_range = max(abs(max(numerical_dpdx)), abs(min(numerical_dpdx)))
            limit = max(data_range, abs(dpdx_theory)) * 1.2
            ax.set_xlim(-limit, limit)
        else:
            ax.set_xlim(-abs(dpdx_theory) * 1.2, abs(dpdx_theory) * 1.2)
        
        ax.set_ylim(self.solver.y_min - 0.1, self.solver.y_max + 0.1)
...
def main():
    ...
    for i in range(1): 
        if 0==0 : 
            ...
            flow.generate_mesh(mesh_type=mesh_type, lc=lc)
