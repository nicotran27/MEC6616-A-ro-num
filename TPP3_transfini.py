...
class VelocityPressurePlotter:
    ...
    def plot_fields(self, u: np.ndarray, v: np.ndarray, p: np.ndarray, 
        ...
        """
        Graphiques des champs de vitesse et de pression.
        """
        ...
        self._plot_u_field(ax1, u, "Champ de vitesse u")
        

        ax2 = fig.add_subplot(gs[0, 1])
        self._plot_v_field(ax2, v, "Champ de vitesse v")
        

        ax3 = fig.add_subplot(gs[0, 2])
        self._plot_velocity_comparison(ax3, u, "Vitesse axial u à la sortie")
        
   
        ax4 = fig.add_subplot(gs[1, 0])
        self._plot_velocity_vectors(ax4, u, v, "Champ vectoriel de vitesse")
        

        ax5 = fig.add_subplot(gs[1, 1])
        self._plot_pressure_field(ax5, p, "Champ de pression")
        

        ax6 = fig.add_subplot(gs[1, 2])
        self._plot_pressure_comparison(ax6, p, "Gradient de pression axial à la sortie")
    ...
    def _get_last_x_position(self):
        """Coordonnée de la dernière cellule en x """
        last_x = float('-inf')
        for center in self.solver.cell_centers:
            if center[0] > last_x:
                last_x = center[0]
        return last_x
    
    def _plot_velocity_comparison(self, ax, u: np.ndarray, subtitle: str):
        """Graphique comparaison avec vitesse analytique"""

        x_target = self._get_last_x_position()
        tolerance = 0.1  
        ...
        for i_elem in range(self.solver.mesh.get_number_of_elements()):
            center = self.solver.cell_centers[i_elem]
            if abs(center[0] - x_target) < tolerance:
                numerical_u.append(u[i_elem])
                numerical_y.append(center[1])
        

        y_analytical = np.linspace(self.solver.y_min, self.solver.y_max, 100)
        u_analytical = []
        for y in y_analytical:
            u_a, _ = self.solver.analytical_solution(x_target, y, 0)
            u_analytical.append(u_a)
        
        # Plot both solutions
        ax.plot(u_analytical, y_analytical, 'r-', label=' Solution analytique')
        ax.scatter(numerical_u, numerical_y, c='b', marker='o', label='Solution numérique')
        
        ax.set_xlabel('Vitesse u')
        ax.set_ylabel('y')
        ax.set_title(f"{subtitle}\n à x = {x_target:.2f}")
        ax.grid(True)
        ax.legend()
    
    def _plot_pressure_comparison(self, ax, p: np.ndarray, subtitle: str):
        """Graphique comparaison avec gradient analytique"""
        tolerance = 1e-6
        
     
        x_positions = [center[0] for center in self.solver.cell_centers]
        unique_x = sorted(list(set([round(x, 6) for x in x_positions])))
        
        x_target = unique_x[-1]  
        x_previous = unique_x[-2]  
        ...
        for i_elem in range(self.solver.mesh.get_number_of_elements()):
            x = self.solver.cell_centers[i_elem][0]
            y = self.solver.cell_centers[i_elem][1]
            ...
            if abs(x - x_target) < tolerance:
                ...
                for j_elem in range(self.solver.mesh.get_number_of_elements()):
                    x_prev = self.solver.cell_centers[j_elem][0]
                    y_prev = self.solver.cell_centers[j_elem][1]
                    
                    if abs(x_prev - x_previous) < tolerance:  
                        dy = abs(y_prev - y)
                        if dy < min_dy:
                            min_dy = dy
                            best_prev = j_elem
                ...
                if best_prev is not None:
                    ...
                    dx = x - self.solver.cell_centers[best_prev][0]
                    if dx > 0:
                        grad_p = (p_target - p[best_prev]) / dx
                        numerical_dpdx.append(grad_p)
                        numerical_y.append(y)
        
       
        y_analytical = np.linspace(self.solver.y_min, self.solver.y_max, 100)
        h = self.solver.y_max - self.solver.y_min
        u_mean = 1.0
        dpdx_theory = -2 * self.solver.mu * u_mean * 6 / (h * h)
        dpdx_analytical = [dpdx_theory] * len(y_analytical)
        
      
        ax.plot(dpdx_analytical, y_analytical, 'r-', label='Analytical')
        if numerical_dpdx:
            ax.scatter(numerical_dpdx, numerical_y, c='b', marker='o', label='Numerical')
            
        
        ax.set_xlabel('Gradient de pression (dp/dx)')
        ax.set_ylabel('y')
        ax.set_title(f"Gradient de pression axial\n à x = {x_target:.2f}")
        ax.grid(True)
        ax.legend()
        
    
        textstr = f'Analytical dp/dx = {dpdx_theory:.2f}'
        props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
        ax.text(0.05, 0.95, textstr, transform=ax.transAxes, fontsize=10,
                verticalalignment='top', bbox=props)
        
   
        margin = abs(dpdx_theory) * 0.3
        if numerical_dpdx:
            actual_max = max(abs(max(numerical_dpdx)), abs(min(numerical_dpdx)))
            limit = max(actual_max, abs(dpdx_theory)) + margin
            ax.set_xlim(-limit, limit)
        else:
            ax.set_xlim(-abs(dpdx_theory) - margin, abs(dpdx_theory) + margin)

    def _plot_u_field(self, ax, u: np.ndarray, subtitle: str):
        """Graphique champ vitesse u"""
        self._plot_mesh(ax)
        patches = []
        for i_elem in range(self.solver.mesh.get_number_of_elements()):
    ...
    def _plot_v_field(self, ax, v: np.ndarray, subtitle: str):
        ...
        ax.add_collection(v_collection)
        plt.colorbar(v_collection, ax=ax, label='Vitesse v')
        
        self._set_plot_properties(ax, subtitle)

    def _plot_velocity_vectors(self, ax, u: np.ndarray, v: np.ndarray, subtitle: str):
        """Graphique champ vecteurs vitesse """
    ...
    def _plot_pressure_field(self, ax, p: np.ndarray, subtitle: str):
        ...
        plt.colorbar(p_collection, ax=ax, label='Pression')
...
def main():
    ...
    for i in range(1): 
        if 0==0 : 
            ...
            plt.title(f'Convergence - {mesh_type} Mesh ({Nx}x{Ny})')
            plt.legend()      
            plt.gcf()
                
            # Visualisation des résultats
            plotter = VelocityPressurePlotter(flow)
            fig = plotter.plot_fields(up, vp, P_field, title="Solution écoulement Poisseuille")
            plt.show()    
